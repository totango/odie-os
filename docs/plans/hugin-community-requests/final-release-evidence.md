...
Strict-runner SHA `56d83de010ec5fb47e4d1f9c6da002a9a56804f7607d7de21ad8326da551f551` was reconfirmed. Exact-tree `pnpm lint` passed, including all 87 build tasks. Exact-tree `pnpm test` passed after isolating and confirming one load-only Context timeout: 196 root tests passed and the Vite+ aggregate completed; affected suites included Sessions 680, frontend 695, Context 49+1, request-build controller 48, and the remaining package tasks. The first GitHub Actions run exposed the documented workerd memory-risk mode: concurrency 4 stopped producing test output and hit the job's 20-minute timeout without a test assertion failure. The root test task now sets Vite+ `--concurrency-limit 2`; a fresh strict-runner run with 0/32 cache hits completed successfully in 148 seconds. `scripts/build-production-deploy.mjs` completed 16/16 production dry-run artifacts with Wrangler `4.131.1`; the temporary artifact directory was removed after verification. Fresh independent code and security reviews both returned PASS with no source/security blocker. GitHub Actions then passed Lint and Build/Test; the concurrency-2 Test step completed in 4m08s. ODI review correctly identified that the credentialed deploy/image jobs still installed candidate workspace dependencies before invoking Wrangler. Both jobs now use pinned `actions/setup-node`, install exact Wrangler `4.131.1` outside the workspace with lifecycle scripts disabled, verify its version, and invoke its absolute path; focused workflow tests pass.
...
Local release closure is complete. PR #166 and workflow-syntax hotfix PR #167 merged with required CI and ODI checks green; main CI passed at `f0e1231827d927da33570f605ffd4d2019cdf4aa`. The first post-main image run built, pushed, and network-none-smoked that commit image, then failed before Cloudflare mirroring because npm 11 rejects loading `/dev/null` as both user and global config. The isolated installer now creates two distinct empty config files under `$RUNNER_TEMP`; local exact-Wrangler installation/version smoke and workflow tests pass. The failed run did not mirror or promote its GHCR candidate. A subsequent run at `79ee00ceea8ff64710a0a826ef9aa1501806d3d8` successfully published, network-none-smoked, and mirrored its image, but all four native canary jobs stopped before deploy because their credentialed jobs inherited no `WRANGLER_BIN`; exact-resource cleanup passed for all tiers. The canary jobs now use the same lifecycle-disabled, exact-version, outside-workspace Wrangler install as publication, while cleanup no longer installs candidate dependencies because it only uses Node and direct control-plane calls. A follow-up run at `286d2a702aaa06ec278fdb610ebd0250e90e4b12` proved the isolated Wrangler install, publication/mirroring, and all-tier cleanup, then exposed that canary bundling still needs the frozen workspace packages. Canary setup now installs that graph with `--frozen-lockfile --ignore-scripts`; Wrangler remains the independently installed absolute binary, and a local dry-run bundled the exact mirrored candidate successfully. Remaining gates are a successful new-commit image publication/native Sandbox canary, genuine live provider/Slack/model/repository checks, migration-aware deployment ordering, and post-deploy smoke/rollback verification. No stale direct Miniflare/Vitest/Sandbox or Hono residual remains from this closure task.
