...
- Release workflow repair follow-up: `publish-coding-session-image` and `deploy-production` no longer install global npm Wrangler or candidate workspace dependencies in credentialed jobs. Privileged steps use pinned `actions/setup-node`, install exact Wrangler `4.131.1` with lifecycle scripts disabled under `$RUNNER_TEMP/wrangler-cli`, verify its version, and invoke its absolute path. Candidate workspace installation/build remains confined to the uncredentialed artifact job.
