...
- `workshop-backend/wrangler.odie-os-production.jsonc:19-26` lists four exact deployment administrators. Post-deploy UI evidence established that Jacob's active persisted account is `jacob.beck@heyodie.ai`, while the stale `jacob.beck@totango.com` entry did not grant that account. The owner explicitly identified the active account, so the production entry is corrected to **jacob.beck@heyodie.ai**; Keith, Nick, and Stacy remain unchanged. This is an exact account replacement, not alias expansion, merging, or a fifth grant.
