...
General internet egress is enabled for coding sessions, so a future DinD profile could reach public image registries. Production profiles must still use reviewed digest-pinned images rather than repository-selected arbitrary pulls. DinD duplicates image/storage overhead and remains a compatibility pool, not the default architecture. Curated native processes or an external ephemeral namespace are preferred.

External ephemeral infrastructure is network-reachable from coding sessions, but its authority still needs an explicit design. Prefer short-lived session capabilities through a reviewed HTTPS/WSS broker instead of placing long-lived database or infrastructure credentials in untrusted session code.
...
These measurements make `standard-3` the initial candidate, subject to a native Cloudflare/Node 24 benchmark. The image needs pinned Node 24.14.x and pnpm 10.22.0; its current base provides Node 22 and no pnpm. The audited root workspace install reads two private GitHub Packages, `@totango/posthog-unified-tracking@0.1.0-alpha.3` and `@totango/unison-core@0.1.6`. Their direct importers are backend workspaces rather than `clients/unison`; a future frozen install filtered to the Unison frontend and its workspace dependency closure might avoid one or both, but that narrower behavior has not been verified. Until it is, the root install requires both exact locked artifacts. Odie must eventually authorize them through a method- and path-restricted Worker-side package credential proxy, using a synthetic sandbox credential. It must not place a reusable `GIT_NPM_TOKEN` in sandbox files or environment variables. The server-private audit manifest remains explicitly incomplete and unavailable until transport limits, redirect behavior, and a supported credential contract are verified. The audited Agentic toolchain uses pnpm 10.22.0 exactly. The evidence-only transport probe does not run pnpm, so package-manager request compatibility remains a separate blocked verification rather than evidence supplied by that probe. Its trusted-launch procedure is documented in the [GitHub Packages transport probe runbook](coding-session-github-packages-probe.md).
...
Readiness and liveness consume bounded restart budgets. Private component log tails share a per-component maximum of 64 KiB and 2,000 lines and are never included in public status. Public errors are fixed server-authored messages. The reader-first foundation understands durable configure/schedule work before its first policy RPC, and an exact generation cancellation tombstone prevents a delayed startup RPC from recreating work after stop. `CODING_SESSION_DURABLE_LIFECYCLE_ENABLED` gates every new start/stop writer. Odie production enables it only after the reader-first foundation is the verified rollback target; generic hosted configuration remains reader-only. Rollback therefore cannot strand a record that the restored Worker does not understand. Once enabled, stop and restart serialize against startup and reconciliation, clean services in reverse topology with confirmed TERM/KILL exits, then cross separate cancel, cleanup, destroy, capacity release/transfer, and scheduling checkpoints. Component failure never destroys the primary terminal. All catalog components, profiles, heavy tiers, images, configuration sources, and previews remain unavailable until their later activation gates are satisfied.
