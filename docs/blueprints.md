...
The first `/api` request a deployment serves installs any whose manifest fingerprint has changed. Format and featured starter fingerprints are tracked separately. Format fingerprints cover title, description, author, revision, and output presentation; starter fingerprints cover title, description, author, and revision. `revision` represents changes to the archive bytes. Each bundled format is promoted only once ever -- an upgrade never undoes an admin's later removal or overrides. Featured starters are installed only into the featured mirror, preserving existing user-featured entries. Starter installation is additive/update-only: omitting an entry from a later build does not remove an already-installed Explore entry, because the mirror also contains user-featured blueprints and does not retain bundle provenance. The ordinary admin feature toggle does not support these ownerless records; retirement requires a deliberate `AdminSettings` maintenance/migration path that removes the exact ID from its authoritative collection and rewrites the snapshot without touching user-featured entries.

Featured starters are deliberately offline-first and currently carry no required blueprint bindings. A non-empty `BlueprintMetadata.bindings` map means the landing page must collect every assignment before creation; that is the wrong contract for a connector a user may skip. Instead, a starter is created immediately and its own UI discovers explicitly wired resources, offers manual/import/demo paths, and persists skip choices. Skip must suppress automatic snapshots as well as explicit sync calls. The Explore page shows the deployment-wide featured snapshot; the Blueprints page shows only the current user's published/uploaded/saved library, so a starter appears there only after the user saves it.

The complete source contract, deterministic packaging rules, optional connector names, security requirements, test checklist, deployment verification, and troubleshooting guide live in [`packages/workshop-backend/featured-blueprints/README.md`](../packages/workshop-backend/featured-blueprints/README.md). Treat that file as the operational source of truth for featured starters.
