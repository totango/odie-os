...
* packages/gatekeeper-odie-kg: Totango Knowledge Graph — a first-party per-user OAuth connector for Agentic's `/api/mcp/odie` resource. Once connected, the account provides an owner-only ambient `TOTANGO_KG` singleton in every workspace. It requests only `mcp:odie:kg:read` plus identity scopes and hard-allows the twelve `odie-kg-*` read tools; exports and skill tools are excluded. Use this source first for customer/account/CSM/product-usage/internal-business questions. It is separate from JARVIS Graphify, which remains the primary source for repository engineering/topology questions.
...
Release pipeline (`scripts/release/`) — how generic customer releases are built for the hosted deployment service. The Totango Odie instance instead deploys directly through `.github/workflows/deploy-production.yml` using the checked-in per-package `wrangler.odie-os-production.jsonc` files:
