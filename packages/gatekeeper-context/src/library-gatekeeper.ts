...
import type {
  VendorDescription, AccountDescription, AgentCatalog, AdminFenceChallenge, AdminFenceEvidence,
  AppUiContext, AuthorizedAppUiContext, GatekeeperUser, GatekeeperUiFrame, ApprovalQueue, ObservationAuthorizer,
...
export class ContextAccount
  ...
  async startAppUi(context: AppUiContext): Promise<GatekeeperUiFrame> {
    await this.#installBundledCollections();
    // Legacy opens retain private management/public reads only. A boolean cannot authorize
    // public mutations; upgraded collection/registry owners also reject old child wire calls.
  ...
  /** Upgraded opens never interpret a boolean as administrator authority. */
  async startAppUiAuthorized(context: AuthorizedAppUiContext): Promise<GatekeeperUiFrame> {
    ...
    await this.#installBundledCollections();
    const ui = new RpcStub(new ContextApiImpl(
      this.env, this.ctx.props.sharingDomain, this.ctx.props.accountId, false,
      this.#collections(), this.#userLibraries(), this.#registries(), context.authorization));
    return {iframeHtml: APP_HTML, ui};
  }
...
export class GatekeeperVendor extends WorkerEntrypoint<Cloudflare.Env, GatekeeperVendorProps> {
  /** Private provider-first evidence using this binding's actual sharing domain, never account props. */
  async adminFenceReadiness(challenge: AdminFenceChallenge): Promise<AdminFenceEvidence> {
    const domain = validateSharingDomain(this.ctx.props.sharingDomain ?? DEFAULT_SHARING_DOMAIN);
