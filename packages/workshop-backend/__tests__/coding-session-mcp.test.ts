...
describe("coding-session MCP resource namespaces", () => {
  ...
  it.each([
    "mcp_portal-42-github-k9x2",
    "mcp_portal-42-resource-k9x2",
  ])("round trips a resource-scoped binding ID %s", bindingId => {
    const namespaced = namespaceCodingSessionResourceUri(bindingId, GENERATION, "ui://widget/index.html");
    expect(parseCodingSessionResourceUri(namespaced)).toEqual({
      bindingId,
      bindingGeneration: GENERATION,
      upstreamUri: "ui://widget/index.html",
    });
  });
