...
it('lets a new account navigate from onboarding to the board without completing setup or connecting services', async () => {
  ...
  const board = createRoute({ getParentRoute: () => base, path: '/requests', component: () => <h1>Community requests reached</h1> })
  ...
  try {
    ...
    expect(container.textContent).toContain('Community requests reached')
