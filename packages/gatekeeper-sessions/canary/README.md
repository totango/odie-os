...
probe waits on a unique ready file. The Worker first obtains a bounded replaying log stream and only
then writes that file, so the process cannot exit before the SDK attaches; the 30-second remote timeout
still bounds it. No later stage, process output or readiness wait,
