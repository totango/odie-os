...
const UPLOADS_UNAVAILABLE = "Uploads unavailable: Team PI provider exposes attachment reads only.";
const INLINE_MEDIA_NOTE = "Images are omitted unless attached through Team PI. Provider-native inline media insertion is not exposed yet.";
...
export function WorkItemEditor({
  ...
  function insertAttachmentReference(attachment: WorkItemEditorAttachment) {
    ...
    setAttachmentPickerOpen(false);
    setStatus(`Inserted ${attachment.name} as a Team PI attachment reference. Provider-native inline media is unavailable.`);
  }
  ...
  return (
    <div className={`work-item-editor ${overLimit ? "invalid" : ""}`} onPasteCapture={blockUnsafeInput} onDropCapture={blockUnsafeInput}>
      <p id={ids.hint} className="sr-only">Markdown editor. Attachments are uploaded through the selected work item's Team PI capability; provider-native inline media is not available.</p>
...
function AttachmentReferencePicker({ attachments, onInsert }: { attachments: WorkItemEditorAttachment[]; onInsert: (attachment: WorkItemEditorAttachment) => void }) {
  return <div className="attachment-reference-picker" role="region" aria-label="Existing attachments">
    <div><strong>Existing attachments</strong><span>Read-only provider assets; insertion creates a Team PI reference, not an upload.</span></div>
...
export function attachmentMarkdownReference(attachment: WorkItemEditorAttachment): string {
  return `[Attachment: ${attachment.name.replace(/[[\]]/g, "") || "file"}](team-pi-attachment://${encodeURIComponent(attachment.id)})`;
}
